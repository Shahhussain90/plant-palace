...
// get all title
let titles = document.querySelectorAll('.row .row .featureCol');
let searchTerm = '';
let tit = '';
...
function search(e) {
  ...
  titles.forEach((a) => {
    ...
    tit = a.textContent.toLowerCase();
    // it search term not in the title's title hide the title. otherwise, show it.
    tit.includes(searchTerm) ? a.style.display = 'block' : a.style.display = 'none';
  });
}
